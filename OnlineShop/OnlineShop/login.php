...
if(isset($_POST['submit']))
{
    ...
    $data = $db->execute_query('SELECT user_id, user_password FROM users WHERE user_login=?', [$_POST['login']])->fetch_assoc();
    if($data['user_password'] === md5(md5($_POST['password'])))
    {
        ...
        $db->execute_query("UPDATE users SET user_hash=?".$insip." WHERE user_id=?", [$hash, $data['user_id']]);
    ...
    else
    {
        ...
        Вы ввели неправильный логин/пароль</div>';
        sleep(1);
    }
}
